...
@section("isi")
	<div class="card">
		  ...
		  <div class="card-footer">
		  	Dipost oleh: <a href="{{url('/admin/user/detail/'.$data->id_user)}}">{{$data->nama}}</a> Tanggal: {{Carbon::parse($data->created_at)->format("d-m-Y")}}
		...
		<div class="card">
		  ...
		  <div class="card-footer">
		  	...
		  	@if(Auth::user()->id_user==$li->id_user)
		  	 	...
		  	 	<a href="{{url('/admin/posting/ubah/'.$li->id_posting)}}" class="btn btn-info">Ubah</a>
		  	 	@endif
		  	 	
		  	@endif
		  		<a href="{{url('/admin/posting/hapus/'.$li->id_posting)}}" class="btn btn-danger">Hapus</a>
		...
		{{$list->links()}}
	@if($data->status_thread == "Buka")
		<form class="form-horizontal" action="{{url('admin/posting/tambah/'.$data->id_thread)}}" method="post">
			<fieldset>
				{{ csrf_field() }}
		@if($errors->any())
